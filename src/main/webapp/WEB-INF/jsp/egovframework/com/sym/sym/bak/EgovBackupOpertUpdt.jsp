...
<title><spring:message code="comSymSymBak.backupOpertUpdt.title"/></title>
...
function fn_egov_save(){
    var varForm = document.getElementById("backupOpert");
    varForm.action = "<c:url value='/sym/sym/bak/updateBackupOpert.do' />";
    ...
    if (varForm.executCycle.value == "02") {
      ...
      if (i == varForm.executSchdulDfkSes.length) {
        alert("<spring:message code="comSymSymBak.backupOpertUpdt.validate.executSchdulDfkSes.length"/>"); //실행주기를 매주로 선택했을 때 요일은 필수 입력값입니다.
        return ;
      }
    }
    ...
    if (varForm.executCycle.value == "03") {
        if (varForm.executSchdulDay.value == "") {
            alert("<spring:message code="comSymSymBak.backupOpertUpdt.validate.cycleMonth.executSchdulDay"/>"); //실행주기가 매월일때 실행스케줄(일)은 필수 입력값입니다.
             return ;
        }
    }
    if (varForm.executCycle.value == "04") {
        if (varForm.executSchdulMonth.value == "")   {
            alert("<spring:message code="comSymSymBak.backupOpertUpdt.validate.cycleYear.executSchdulMonth"/>"); //실행주기가 매년일때 실행스케줄(월)은 필수 입력값입니다.
             return ;
        }
        if (varForm.executSchdulDay.value == "") {
            alert("<spring:message code="comSymSymBak.backupOpertUpdt.validate.cycleYear.executSchdulDay"/>"); //실행주기가 매년일때 실행스케줄(일)은 필수 입력값입니다.
             return ;
        }
    ...
    if ( varForm.executCycle.value == "05" )  {
        ...
        if (varForm.executSchdulDeNm.value == "")   {
            alert("<spring:message code="comSymSymBak.backupOpertUpdt.validate.executSchdulDeNm"/>"); //실행주기가 한번만일때 실행스케줄(일자)는 필수 입력값입니다.
             return ;
        }
        if (!isDate(varForm.executSchdulDeNm.value, "<spring:message code="comSymSymBak.backupOpertUpdt.validate.executSchdulDeNm.isDate"/>"))  { //실행스케줄(일자)가 유효하지 않습니다.
             return ;
        }
    }
...
<c:set var="vexecutSchdulHour"><spring:message code="comSymSymBak.backupOpertUpdt.executSchdulHour"/></c:set>
<c:set var="vexecutSchdulMnt"><spring:message code="comSymSymBak.backupOpertUpdt.executSchdulMnt"/></c:set>
<c:set var="vexecutSchdulSecnd"><spring:message code="comSymSymBak.backupOpertUpdt.executSchdulSecnd"/></c:set>

<form:form commandName="backupOpert"  action="<c:url value='/sym/sym/bak/updateBackupOpert.do' />" method="post">
...
<noscript class="noScriptTitle"><spring:message code="common.noScriptTitle.msg" /></noscript><!-- 자바스크립트를 지원하지 않는 브라우저에서는 일부 기능을 사용하실 수 없습니다. -->
	...
	<!-- 타이틀 -->
	<h2><spring:message code="comSymSymBak.backupOpertUpdt.pageTop.title"/></h2><!-- 백업작업 수정 -->
	...
	<table class="wTable">
		...
		<tr>
			<th><spring:message code="comSymSymBak.backupOpertUpdt.backupOpertId"/> <span class="pilsu">*</span></th><!-- 백업작업ID -->
			<td class="left">
			    <form:input path="backupOpertId" maxlength="20" readonly cssClass="readOnlyClass" cssStyle="width:138px"/>
        		<form:errors path="backupOpertId" cssClass="error" />
			</td>
		</tr>
		<tr>
			<th><spring:message code="comSymSymBak.backupOpertUpdt.backupOpertNm"/> <span class="pilsu">*</span></th><!-- 백업작업명 -->
			<td class="left">
		...
		<tr>
			<th><spring:message code="comSymSymBak.backupOpertUpdt.backupOrginlDrctry"/> <span class="pilsu">*</span></th><!-- 백업원본디렉토리 -->
			<td class="left">
		...
		<tr>
			<th><spring:message code="comSymSymBak.backupOpertUpdt.backupStreDrctry"/> <span class="pilsu">*</span></th><!-- 백업저장디렉토리 -->
			<td class="left">
		...
		<tr>
			<th><spring:message code="comSymSymBak.backupOpertUpdt.executCycleNm"/> <span class="pilsu">*</span></th><!-- 실행주기 -->
			<td class="left">
				...
				<span id="spnYyyyMMdd">
					...
					<input name="executSchdulDeNm" id="executSchdulDeNm" type="text" maxlength="10" title="<spring:message code="comSymSymBak.backupOpertUpdt.executSchdulDeNm"/>" readonly style="width:68px; vertical-align:top" /><!-- 실행스케줄(일자) -->
				</span>
				<span id="spnMonth">
					<select name="executSchdulMonth" id="executSchdulMonth" title="<spring:message code="comSymSymBak.backupOpertUpdt.executSchdulMonth"/>"><!-- 실행스케줄(월) -->
					...
					</select> <spring:message code="comSymSymBak.backupOpertUpdt.spnMonth"/><!-- 월 -->
				</span>
				<span id="spnDay">
					<select name="executSchdulDay" id="executSchdulDay" title="<spring:message code="comSymSymBak.backupOpertUpdt.executSchdulDay"/>"><!-- 실행스케줄(일) -->
						<option value="" selected="selected"><spring:message code="input.cSelect"/></option><!-- 선택 -->
						<c:forEach var="i" begin="1" end="31" step="1">
						<c:if test="${i < 10}"><option value="0${i}">0${i}</option></c:if>
						<c:if test="${i >= 10}"><option value="${i}">${i}</option></c:if>
						</c:forEach>
					</select> <spring:message code="comSymSymBak.backupOpertUpdt.spnDay"/><!-- 일 -->
				</span>
				<br />
				<span id="spnDfk">
				...
				<span id="spnHHmmss">
					<form:select path="executSchdulHour" title="${vexecutSchdulHour}"><!-- 실행스케줄(시) -->
						<form:options items="${executSchdulHourList}" />
					</form:select>
					<form:errors path="executSchdulHour" cssClass="error"/> <spring:message code="comSymSymBak.backupOpertUpdt.spnHH"/><!-- 시 -->
					<form:select path="executSchdulMnt" title="${vexecutSchdulMnt}"><!-- 실행스케줄(분) -->
						<form:options items="${executSchdulMntList}" />
					</form:select>
					<form:errors path="executSchdulMnt" cssClass="error"/> <spring:message code="comSymSymBak.backupOpertUpdt.spnMM"/><!-- 분 -->
					<form:select path="executSchdulSecnd"  title="${vexecutSchdulSecnd}"><!-- 실행스케줄(초) -->
						<form:options items="${executSchdulSecndList}" />
					</form:select>
					<form:errors path="executSchdulSecnd" cssClass="error"/> <spring:message code="comSymSymBak.backupOpertUpdt.spnSS"/><!-- 초 -->
				</span>
			</td>
		</tr>
		<tr>
			<th><spring:message code="comSymSymBak.backupOpertUpdt.cmprsSeNm"/> <span class="pilsu">*</span></th><!-- 압축구분 -->
			<td class="left">

...
function fncSelectGroup(groupId, groupNm) {
 // window.returnValue = groupId;
	if(typeof parent.window['dialogCallback'] === 'function') {
		parent.window['dialogCallback'](groupId, groupNm)
	}else{
...
<div class="popup">
	...
	<tr>
		...
		<td class="lt_text" nowrap="nowrap"><a href="#LINK" onclick="javascript:fncSelectGroup('<c:out value="${group.groupId}"/>', '<c:out value="${group.groupNm}"/>')"><c:out value="${group.groupId}"/></a></td>

...
(function ($) {
    ...
    $.fn.uploadFile = function (options) {
        ...
        var s = $.extend({
            ...
            dragDropStr: "<span><b>여기로 파일을 드래그 하세요</b></span>",
            uploadStr:"파일업로드",
            abortStr: "Abort",
            cancelStr: "Cancel",
            deleteStr: "Delete",
        ...
        var tempAttachId = s.atchFileId ? s.atchFileId : '';
       // console.log("===>"+tempAttachId)
	...
	this.enqueueFile = function(file){
        ...
        this.createProgress = function (dataObj) {


            var url ="/"+s.target+"/info/file/ImageView.do?atchFileId="+dataObj.atchFileId+"&fileSn="+dataObj.fileSn;
            ...
            if(s.showFileCounter)
            	fileNameStr = obj.fileCounter + s.fileCounterStyle + dataObj.orignlFileNm;
            else fileNameStr = dataObj.orignlFileNm;

          if(s.showFileSize)
				fileNameStr += " ("+getSizeStr(dataObj.fileSize)+")</br>"+url;
          ...
          pd.filename.html(fileNameStr);
            ...
            if(s.showPreview)
            {
                var ext = dataObj.fileExtsn.toLowerCase();
                if($.inArray(ext, ['gif','png','jpg','jpeg']) == -1) {

                }

                pd.preview.attr('src',s.viewUrl+"?atchFileId="+dataObj.atchFileId+"&fileSn="+dataObj.fileSn);
                pd.preview.show();
            }
            ...
            if(s.showDownload) {
                ...
                pd.download.click(function () {
                    if(s.downloadCallback) s.downloadCallback.call(obj, dataObj, pd);
                });
            }
            if(s.showDelete)
            {
        ...
        function serializeAndUploadFiles(s, obj, files) {
            ...
            for(var i = 0; i < files.length; i++) {
                ...
                fd.append("attachId",tempAttachId);
        ...
        function ajaxFormSubmit(form, s, pd, fileArray, obj, file) {
            ...
            var options = {
                ...
                type: s.method,
                 data: s.formData,
                // data: JSON.stringify({atchFileId:tempAttachId}),
                formData: s.fileData,
                    ...
                    if(s.onSubmit.call(this, fileArray) != false) {
                        ...
                        if(s.dynamicFormData)
                        {
                            ...
                            if(sData) {
                                for(var j = 0; j < sData.length; j++) {
                                    if(sData[j]) {
                                        if(s.serialize && s.fileData != undefined){
                ...
                success: function (data, message, xhr) {
                    ...
                    if(tempAttachId==null||tempAttachId==""){

                        tempAttachId =data.atchFileId;
                    }
                 	pd.cancel.remove();
                	progressQ.pop();
                    //For custom errors.
